    ...
    def forward(self, latents_in, depth, alpha, labels_in=None):
        ...
        if self.training:
            ...
            if self.style_mixing_prob is not None and self.style_mixing_prob > 0:
                latents2 = torch.randn(latents_in.shape).cuda()
                dlatents2 = self.g_mapping(latents2)
                layer_idx = torch.from_numpy(np.arange(self.num_layers)[np.newaxis, :, np.newaxis]).cuda()
...
class Discriminator(nn.Module):
    ...
    def __init__(self, resolution, num_channels=3, conditional=False,
        ...
        self.final_block = DiscriminatorTop(self.mbstd_group_size, self.mbstd_num_features,
                                            ...
                                            gain=gain,resolution=resolution, use_wscale=use_wscale, activation_layer=act)
...
class StyleGAN:
    ...
    def train(self, dataset, num_workers, epochs, batch_sizes, fade_in_percentage, output,
        ...
        fixed_input = torch.randn(num_samples, self.latent_size).cuda()
        
        fixed_labels = None
        if self.conditional:
            fixed_labels = torch.linspace(
                0, self.n_classes - 1, num_samples).to(torch.int64).cuda()
        ...
        for current_depth in range(start_depth, self.depth):
            ...
            for epoch in range(1, epochs[current_depth] + 1):
                ...
                for i, batch in enumerate(data, 1):
                    ...
                    images = images.cuda()
                    gan_input = torch.randn(images.shape[0], self.latent_size,dtype=torch.float32).cuda()
